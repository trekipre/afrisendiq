...
revoke all on table public.internal_settings from anon, authenticated;
grant select, insert, update on table public.internal_settings to service_role;

insert into public.internal_settings (key, value)
values (
  'manual_billing_alerts',
  jsonb_build_object('stuckPaidThresholdMinutes', 20)
)
